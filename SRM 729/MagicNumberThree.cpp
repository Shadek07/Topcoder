...
class MagicNumberThree {
    ...
    int countSubsequences(string s)
    {
        ...
        forl(i,0,n){
            ...
            forl(j,0,3)
              tmp[j] = dp[j];
            tmp[a%3] = (dp[a%3] + 1)%1000000007; //start of new sequence
            forl(j,0,3){ //for every previous remainder add current digit to make new subsequence
                int b = (j*10 + a)%3;
                tmp[b] = (dp[b] + dp[j])%1000000007; //b is new remainder being formed
            }
            forl(j,0,3)
            dp[j] = tmp[j];
        }
