...
bool dp[31][31][31][440];
string ans;
int n;
int match;
bool found;

void doit(int a, int b, int len, int pa, string s)
{
	if(found)
		return;
    int i,j,k;
	...
	doit(a+1,b,len+1,pa+b+c, 'A' + s);
	doit(a,b+1,len+1,pa+c, 'B' + s);
	doit(a,b,len+1,pa, 'C' + s);
