...
long long int doit(int i, int j){
    ...
    if(res == -1){
        res = 0;
        res = doit(i+1,j); //do not include i as part of the bracket sequence
        forl(k,i+1,j+1){ //pair i with k and divide the sequence in two parts
            if((str[i] == '(' && str[k] == ')') || (str[i] == '[' && str[k] == ']')){
                res += (1+doit(i+1,k-1))*(1+doit(k+1,j));
            }
        }
    }
    return res;
