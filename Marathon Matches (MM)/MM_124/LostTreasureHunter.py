...
class LostTreasureHunter :
    ...
    def backandforthTrip(self):
        zero_treasure = False
        while not zero_treasure:
            print(-1)
            sys.stdout.flush()
            t = int(input())
            pathCount = int(input())
            time = input()
            self.treasure_history.append(t)
            take = min(t, self.maxTreasure)     
            self.total_treasures += take
            self.prev_val = self.finalvalue()
            self.numStep += 1
            print(take)
            if take == 0:
                zero_treasure = True
    ...
    def processChamber(self, t: int, p: int):
        ...
        '''if new_value < self.prev_val*0.70:
            print(-1)
            sys.stdout.flush()'''
        print(take)
        prob = random.randint(1,100)
        #if prob <= 50 and len(self.treasure_history) > 1 and self.treasure_history[-2] > self.treasure_history[-1]*4:
        if  len(self.treasure_history) > 1 and self.treasure_history[-2] > self.treasure_history[-1]*2 and self.treasure_history[-1]>0:
            self.backandforthTrip()
            return
